...
public class word2Vec 
{
	public static void main(String[] args) throws IOException, ParseException 
	{
		try {
			...
			// Declares SentenceIterator
			FieldValuesSentenceIterator iter = new FieldValuesSentenceIterator(reader, "text");

			// Model, window, layer size to be used into the model creation.
			SkipGram model = new SkipGram<>();
			Word2Vec vec = new Word2Vec.Builder()
					.layerSize(100)
					.windowSize(5)
					.iterate(iter)
					.elementsLearningAlgorithm(model)
					.build();
